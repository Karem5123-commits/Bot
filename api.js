'use strict';
// =============================================================
// API SERVER v5 — ULTRA EDITION (100x POWER)
...
const {
  ...
  getUser, updateUser, addBalance, getJackpot,
  getRankFromElo, isCommandDisabled, setCommandEnabled,
  ALL_COMMANDS, assignAutoRoleToAll, trackEvent, RANKS,
} = require('./main');
...
app.use((req, res, next) => {
  apiRequests++;
  res.setHeader('X-Powered-By', 'GOD MODE BOT v5');
  next();
});
...
// =============================================================
const wsClients = new Map(); // ws -> { ip, connectedAt, isAuthed }
...
wss.on('connection', (ws, req) => {
  ...
  ws.send(JSON.stringify({
    event: 'connected',
    data: { message: 'GOD MODE BOT v5 Live Feed', clients: wsClients.size },
    timestamp: Date.now(),
  }));

  // Allow WS auth for admin-only events
  ws.on('message', raw => {
    try {
  ...
  ws.on('close', () => {
    wsClients.delete(ws);
    log('API', `WS disconnected (${wsClients.size} remaining)`);
  });
...
/** Validate ObjectId-safe userId (Discord snowflake) */
function isValidUserId(id) {
  return typeof id === 'string' && /^\d{17,20}$/.test(id);
}

/** Parse integer with fallback and bounds */
...
function adminAuth(req, res, next) {
  ...
  try {
    ...
    if (err.name === 'TokenExpiredError') return sendError(res, 401, 'Token expired. Please log in again.');
    return sendError(res, 401, 'Invalid token.');
  }
}

// =============================================================
// ASYNC ROUTE WRAPPER — catches all async errors
// =============================================================
...
app.get('/api/dashboard', asyncRoute(async (req, res) => {
  const [totalUsers, totalSubs, pending, jackpot, totalBets, totalGiveaways] = await Promise.all([
    ...
    getJackpot(),
    db.collection('command_logs').countDocuments({
      command: { $in: ['coinflip','slots','roulette','blackjack','dice','spin','allin','bet'] },
    }),
    db.collection('giveaways').countDocuments(),
  ]);
  res.json({
    totalUsers, totalSubmissions: totalSubs, pendingReviews: pending,
    jackpot, totalBets, totalGiveaways,
    uptime:    Math.floor((Date.now() - startTime) / 1000),
    cacheSize: userCache.size,
    guilds:    client.guilds.cache.size,
    members:   client.guilds.cache.reduce((a, g) => a + g.memberCount, 0),
...
app.get('/api/user/:userId', asyncRoute(async (req, res) => {
  ...
  // Omit sensitive fields from public endpoint
...
app.get('/api/submissions', asyncRoute(async (req, res) => {
  ...
  const skip  = parseIntBounded(req.query.skip, 0, 0, 100_000);

  const [subs, total] = await Promise.all([
    db.collection('submissions').find(filter)
      .sort({ submittedAt: -1 }).skip(skip).limit(limit).toArray(),
...
app.get('/api/giveaways', asyncRoute(async (req, res) => {
  ...
  const gws = await db.collection('giveaways')
...
app.get('/api/commands/list', (req, res) => {
  res.json({
    ...
    disabledCount: CONFIG.disabledCommands.size,
  });
});
...
app.post('/api/admin/login', authLimiter, asyncRoute(async (req, res) => {
  ...
  if (!userId || !password)
    return sendError(res, 400, 'Missing userId or password.');
  ...
  if (!CONFIG.ownerIds.includes(userId)) {
    log('ADMIN', `Login rejected — not an owner: ${userId}`);
    await trackEvent('admin_login_fail', { userId, reason: 'not_owner' });
    return sendError(res, 403, 'Not authorized.');
  }
  if (password !== CONFIG.adminPassword) {
    log('ADMIN', `Wrong password for ${userId}`);
    await trackEvent('admin_login_fail', { userId, reason: 'wrong_password' });
    return sendError(res, 401, 'Invalid password.');
  }

  const token = jwt.sign(
    { userId, isOwner: true, iat: Math.floor(Date.now() / 1000) },
    CONFIG.jwtSecret,
    { expiresIn: '24h' }
  );
...
app.post('/api/admin/verify', adminAuth, (req, res) => {
  res.json({
    valid:  true,
    user:   req.admin,
    server: { uptime: Math.floor((Date.now() - startTime) / 1000), version: '5.0.0' },
  });
});

app.post('/api/admin/logout', adminAuth, asyncRoute(async (req, res) => {
  // In a real system you'd blacklist the token; for now just log
  log('ADMIN', `Owner ${req.admin.userId} logged out`);
  await trackEvent('admin_logout', { userId: req.admin.userId });
...
app.post('/api/admin/commands/toggle', adminAuth, asyncRoute(async (req, res) => {
  ...
  if (typeof command !== 'string' || !ALL_COMMANDS.includes(command))
    return sendError(res, 400, `Invalid command. Valid: ${ALL_COMMANDS.join(', ')}`);
...
app.post('/api/admin/commands/bulk-toggle', adminAuth, asyncRoute(async (req, res) => {
  ...
  if (!Array.isArray(commands) || commands.length === 0)
    return sendError(res, 400, 'commands must be a non-empty array.');
  if (typeof enabled !== 'boolean')
    return sendError(res, 400, 'enabled must be boolean.');

  const valid = commands.filter(c => ALL_COMMANDS.includes(c));
  const invalid = commands.filter(c => !ALL_COMMANDS.includes(c));

  await Promise.all(valid.map(cmd => setCommandEnabled(cmd, enabled)));
  ...
  res.json({ success: true, affected: valid.length, invalid });
}));
...
app.post('/api/admin/user/balance', adminAuth, asyncRoute(async (req, res) => {
  ...
  if (!['set','add','subtract'].includes(operation))
    return sendError(res, 400, 'operation must be set | add | subtract.');
...
app.post('/api/admin/user/elo', adminAuth, asyncRoute(async (req, res) => {
  ...
  if (!isValidUserId(userId))    return sendError(res, 400, 'Invalid userId.');
  if (typeof elo !== 'number')   return sendError(res, 400, 'elo must be a number.');
...
app.post('/api/admin/user/ban', adminAuth, asyncRoute(async (req, res) => {
  ...
  await updateUser(userId, {
    ...
    bannedBy:  banned ? req.admin.userId : null,
  });
...
app.patch('/api/admin/user/:userId/level', adminAuth, asyncRoute(async (req, res) => {
  ...
  const update = {};
  if (typeof level === 'number') update.level = Math.max(1, Math.floor(level));
  if (typeof xp === 'number')    update.xp    = Math.max(0, Math.floor(xp));
...
app.post('/api/admin/user/dm', adminAuth, asyncRoute(async (req, res) => {
  ...
  if (!isValidUserId(userId)) return sendError(res, 400, 'Invalid userId.');
  if (!msg || typeof msg !== 'string') return sendError(res, 400, 'message is required.');
...
app.get('/api/admin/users/search', adminAuth, asyncRoute(async (req, res) => {
  const q      = (req.query.q || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); // escape regex
  ...
  const users  = await db.collection('users')
    .find(filter).sort({ [sortBy]: -1 }).limit(limit).toArray();
  res.json(users);
}));
...
app.post('/api/admin/broadcast', adminAuth, asyncRoute(async (req, res) => {
  ...
  if (!msg) return sendError(res, 400, 'message is required.');
  ...
  for (const guild of targetGuilds) {
    const ch = guild.channels.cache.find(c =>
      c.isTextBased() && c.name === (channelName || 'general')
    );
    if (ch) {
      const ok = await ch.send(msg.slice(0, 2000)).then(() => true).catch(() => false);
      ok ? sent++ : failed++;
    }
  }
...
app.post('/api/admin/jackpot/set', adminAuth, asyncRoute(async (req, res) => {
  ...
  if (typeof amount !== 'number' || amount < 0)
    return sendError(res, 400, 'amount must be a non-negative number.');
  const safeAmount = Math.floor(amount);
  await db.collection('jackpot').updateOne(
    { id: 'main' }, { $set: { pool: safeAmount } }, { upsert: true }
  );
...
app.post('/api/admin/member/kick', adminAuth, asyncRoute(async (req, res) => {
  ...
  if (!guildId || !isValidUserId(userId))
    return sendError(res, 400, 'guildId and valid userId are required.');
  const guild  = client.guilds.cache.get(guildId);
  if (!guild)  return sendError(res, 404, 'Guild not found.');
...
app.post('/api/admin/member/ban', adminAuth, asyncRoute(async (req, res) => {
  ...
  await guild.members.ban(userId, {
    reason:             reason || 'Via dashboard',
    deleteMessageSeconds: Math.min(604800, Math.max(0, (deleteMessageDays || 0) * 86400)),
  });
...
app.post('/api/admin/member/timeout', adminAuth, asyncRoute(async (req, res) => {
  ...
  if (!guild)  return sendError(res, 404, 'Guild not found.');
  const member = await guild.members.fetch(userId);
  const ms     = minutes ? Math.min(40320, Math.max(1, minutes)) * 60_000 : null;
...
app.post('/api/admin/presence', adminAuth, asyncRoute(async (req, res) => {
  ...
  client.user.setPresence({
    ...
    activities: activity
      ? [{ name: activity.slice(0, 128), type: typeMap[type] ?? 0 }]
      : [],
  });
  log('ADMIN', `${req.admin.userId} updated bot presence`);
  res.json({ success: true, status: status || 'online', activity, type });
}));
...
app.post('/api/admin/config/update', adminAuth, asyncRoute(async (req, res) => {
  const allowed = [
    'prefix', 'autoDeleteSeconds', 'jackpotCut', 'autoRoleId',
    'reviewChannelId', 'logChannelId', 'maxBet', 'minBet',
    'dailyAmount', 'dailyStreakBonus',
  ];
  const changed = {};
  ...
  await db.collection('settings').updateOne(
    { key: 'bot_config' },
    { $set: { value: changed, updatedAt: new Date(), updatedBy: req.admin.userId } },
    { upsert: true }
  );
...
app.get('/api/admin/db/collections', adminAuth, asyncRoute(async (req, res) => {
  ...
  const stats = Object.fromEntries(
    await Promise.all(
      collections.map(async c => [c.name, await db.collection(c.name).countDocuments()])
    )
  );
  res.json(stats);
}));
...
app.delete('/api/admin/db/clear/:collection', adminAuth, asyncRoute(async (req, res) => {
  ...
  if (!allowed.includes(req.params.collection))
    return sendError(res, 403, `Not allowed. Clearable: ${allowed.join(', ')}`);
...
app.post('/api/admin/ranks/recalc', adminAuth, asyncRoute(async (req, res) => {
  ...
  for (const u of users) {
    ...
    if (correctRank !== u.rank) {
      bulkOps.push({
        updateOne: {
          filter: { userId: u.userId },
          update: { $set: { rank: correctRank } },
        },
      });
      updated++;
    }
...
app.post('/api/admin/submissions/:id/review', adminAuth, asyncRoute(async (req, res) => {
  ...
  const { ObjectId } = require('./main');
  let oid;
  try { oid = new ObjectId(req.params.id); }
  catch { return sendError(res, 400, 'Invalid submission ID.'); }
  ...
  if (!sub) return sendError(res, 404, 'Submission not found or already reviewed.');
...
app.post('/api/admin/system/gc', adminAuth, (req, res) => {
  if (global.gc) {
    global.gc();
    log('ADMIN', `${req.admin.userId} triggered GC`);
    res.json({ success: true, message: 'Garbage collection triggered.' });
  } else {
    res.json({ success: false, message: 'GC not exposed. Start node with --expose-gc.' });
  }
});
...
module.exports = function startApiServer() {
  ...
  server.on('error', err => {
    if (err.code === 'EADDRINUSE') {
      log('ERROR', `Port ${CONFIG.port} already in use. Set PORT env var to a different port.`);
      process.exit(1);
    }
