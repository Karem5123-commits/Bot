    ...
    try {
      ...
      if (interaction.isButton() && ['bj_hit','bj_stand'].includes(interaction.customId)) {
        ...
        if (interaction.customId === 'bj_hit') {
          ...
          return interaction.update({
            embeds: [new EmbedBuilder().setColor(0x1A1A2E).setTitle('🃏 Blackjack').addFields(
      ...
      if (interaction.isButton() && interaction.customId === 'open_ticket') {
        ...
        const embed = new EmbedBuilder().setColor(0x5865F2).setTitle('🎫 Support Ticket')
          .setDescription(`Hello <@${interaction.user.id}>! Staff will assist shortly.`);
        const row = new ActionRowBuilder().addComponents(
      ...
      if (interaction.isButton() && interaction.customId.startsWith('gw_enter_')) {
        ...
        if (!gw || gw.ended) return interaction.reply({ content: '❌ Ended.', ephemeral: true });
        if (gw.entries.includes(interaction.user.id))
          return interaction.reply({ content: 'Already entered!', ephemeral: true });
        await db.collection('giveaways').updateOne({ _id: gw._id }, { $push: { entries: interaction.user.id } });
      ...
      if (interaction.isButton() && interaction.customId.startsWith('rate_')) {
        ...
        const eloGain = calcElo(rating, td.elo, td.streak || 0);
      ...
      if (interaction.isChatInputCommand()) {
        ...
        if (interaction.commandName === 'submit') {
          ...
          modal.addComponents(
            ...
            new ActionRowBuilder().addComponents(
              new TextInputBuilder().setCustomId('clip_desc').setLabel('Description').setStyle(TextInputStyle.Paragraph).setRequired(false)
            )
          );
        ...
        if (interaction.commandName === 'review') {
          ...
          if (!pending.length) return interaction.reply({ content: '✨ No pending!', ephemeral: true });
          const sub = pending[0];
        ...
        if (interaction.commandName === 'verify_panel') {
          if (!isStaff) return interaction.reply({ content: 'Staff only.', ephemeral: true });
          const embed = new EmbedBuilder().setColor(0x22c55e).setTitle('✅ Verification')
        ...
        if (interaction.commandName === 'suggestion') {
          ...
          const embed = new EmbedBuilder().setColor(0xFFFF00).setTitle('💡 Suggestion')
            .setDescription(text)
        ...
        if (interaction.commandName === 'shop') {
          const embed = new EmbedBuilder().setColor(0x5865F2).setTitle('🛒 Shop')
      ...
      if (interaction.isModalSubmit() && interaction.customId === 'submit_modal') {
        ...
        if (reviewCh) {
          const embed = new EmbedBuilder().setColor(0xFF6B6B).setTitle('📨 New Submission')
            .addFields(
      ...
      log('ERROR', `Interaction: ${err.message}`);
      try {
